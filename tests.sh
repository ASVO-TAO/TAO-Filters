...
find . -type f \( -iname \*.dat -o -iname \*.dati \) -exec awk '{if(NR==1) firstline=$0;} END{if(NR-1 != firstline) {print "In file ", ARGV[1], ": The first line = ",firstline," does not equal the total number of lines with filter data = ",NR-1 ; exit 1;}}' {} \;
